<footer>
    <div class="container links">
        <div class="row">
            <div class="col-lg-4 col-md-4 col-md-4 col-sm-4 col-xs-12">
                <div class="footer-link-heading">Group Members</div>
                <ul>
                    <li><a href="http://josephmaxim.io/">Joseph Dagunan</a></li>
                    <li><a href="http://housing.davidmichaelbond.com/login.php">David Bond</a></li>
                    <li><a href="#">Alex Waddell</a></li>
                </ul>
            </div>
            <div class="col-lg-4 col-md-4 col-md-4 col-sm-4 col-xs-12">
                <div class="footer-link-heading">Useful Links</div>
                <ul>
                    <li><a href="http://opentech2.durhamcollege.org/pufferd/webd2201/">WEBD2201</a></li>
                    <li><a href="http://www.w3schools.com">W3Schools</a></li>
                    <li><a href="http://validator.w3.org">XHTML Validator</a></li>
                    <li><a href="http://jigsaw.w3.org/css-validator/">CSS Validator</a></li>
                    <li><a href="http://php.net/manual/en/index.php">PHP Manual</a></li>
                </ul>
            </div>
            <div class="col-lg-4 col-md-4 col-md-4 col-sm-4 col-xs-12">
                <div class="footer-link-heading">School Websites</div>
                <ul>
                    <li><a href="http://www.durhamcollege.ca/">Durham College</a></li>
                    <li><a href="http://www.durhamcollege.ca/connect/">DC Connect</a></li>
                    <li><a href="http://www.durhamcollege.ca/mycampus/">MyCampus</a></li>
                </ul>
            </div>
        </div>
    </div>
    <div class="credits">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <span>&copy; 2016 Cloud Estate. All Rights Reserved.</span>
                </div>
                <div class="col-lg-6 text-right">
                    <p>
                        <a href="https://validator.w3.org/nu/?doc=http%3A%2F%2Fopentech2.durhamcollege.org%2Fwebd3201%2Fgroup15%2F">
                            <img style="width:50px; height:20px;" src="http://as76.net/com/img/valid-html5.png" alt="Valid XHTML 1.0 Strict" />
                        </a>
                        <a href="http://jigsaw.w3.org/css-validator/check/referer">
                            <img style="width:50px; height:20px;" src="http://jigsaw.w3.org/css-validator/images/vcss" alt="Valid CSS!" />
                        </a>
                    </p>
                </div>
            </div>
        </div>
    </div>
</footer>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
<script src="js/bootstrap.min.js"></script>
</body>
</html>